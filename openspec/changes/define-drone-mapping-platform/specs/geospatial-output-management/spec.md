...
系统 SHALL 为每项成果记录类型、版本、坐标参考系、范围、分辨率或点密度、文件大小、生成任务、引擎与参数版本、精度等级和质量报告。系统 SHALL 将同一逻辑成果组织为成果系列，并为每次生成建立不可变成果版本；浏览派生物、导出包、网格 3D Tiles 和 Cesium 高斯 3D Tiles SHALL 作为独立成果版本引用其来源正式成果，可以失效或重建但不得覆盖来源正式成果。
